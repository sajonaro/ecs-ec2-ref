...
module "dns-and-certificate-settings" {
  ...
  forward_to_dns_name = module.ecs-cluster-configuration.alb_dns_name
  forward_to_zone_id = module.ecs-cluster-configuration.alb_zone_id
}
...
module "blue-task-definition" {
  ...
  task_definition_name           = local.task_definition_name_blue
...
module "green-task-definition" {
  ...
  task_definition_name           = local.task_definition_name_geen
  host_port                      = local.host_port
  aws_region                     = local.region
  ecs_task_execution_role_arn    = module.task-execution-role.task_execution_role_arn
  container_path                 = local.container_path
...
module "ecs-cluster-configuration" {
  source = "./modules/ecs-config"
...
module "capacity-provider" {
  source               = "./modules/cp"
  vpc_id               = module.ecs-cluster-configuration.vpc_id
  subnet_ids           = module.ecs-cluster-configuration.vpc_az_ids
  ...
  alb_sg_id            = module.ecs-cluster-configuration.alb_sg_id
...
module "bastion-host" {
  ...
  subnet_id         = module.ecs-cluster-configuration.public_subnet_id
  app_name          = local.app_cluster_name
}